...
bool TCPSocketManager::Connected()
{
	if(pSockStrand->running_in_this_thread())
		return isConnected;

	auto tracker = handler_tracker;
	std::atomic_bool executed = false;
	bool result;
	pSockStrand->post([this,tracker,&executed,&result]()
		{
			result = isConnected;
			executed = true;
		});
...
{
	auto tracker = handler_tracker;
	auto pTimer = std::make_shared<asio::steady_timer>(IOS);
...
bool TCPSocketManager::Open(std::shared_ptr<void> tracker)
{
	...
	for(auto endpoint_it = EndpointIterator; endpoint_it != end; endpoint_it++)
	{
		...
		auto pCandidateSock = std::make_shared<asio::ip::tcp::socket>(IOS);
...
bool TCPSocketManager::EndPointResolved(std::shared_ptr<void> tracker)
{
	...
	if(EndpointIterator == end || (msSinceEpoch()-resolve_time) > 900000)
	{
		...
		auto res = std::make_shared<asio::ip::tcp::resolver>(IOS);
...
void TCPSocketManager::ServerOpen(std::shared_ptr<asio::ip::tcp::socket> pCandidateSock, asio::ip::tcp::resolver::iterator endpoint_it, std::string addr_str, std::shared_ptr<void> tracker)
{
	try
	{
		pAcceptor = std::make_unique<asio::ip::tcp::acceptor>(IOS,*endpoint_it);
